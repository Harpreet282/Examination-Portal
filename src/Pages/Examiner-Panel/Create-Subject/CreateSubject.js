...
function CreateSubject() {
  ...
  const chaneHandler = (e) => {
    setSubjectName((oldData) => ({ ...oldData, [e.target.name]: e.target.value }));
  };
  ...
  const submitHandler = (event) => {
    ...
    axios.post(ADD_SUBECT, { subjects }, { headers: { Authorization: `Bearer ${token}` } })
      .then((res) => {
        const { data } = res;
        console.log(data);
      })
