...
const GiveExam = () => {
  ...
  const [toggleNo, setToggleNo] = useState(-2);
  const screen1 = useFullScreenHandle();

//   console.log(screen1, ">>>>>>>>>>>>>> screen 1");
  ...
  let mins = parseInt(data.exam.duration.split(":")[1])
    ? parseInt(data.exam.duration.split(":")[1]) * 60000
    : 0;
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `https://exam-portal-by-hritik-sanam.herokuapp.com/student/questions?studentID=${data.studentID}&examID=${data.exam.examID}&pageIndex=${pageNo}`,
        {
      ...
      .then((res) => {
        ...
        setCheckList(
          Array(res.data.data.totalPages).fill(Array(4).fill(false))
        );
      })
      .catch((err) => {
        dispatch(loaderValueFalse());
  ...
  useMemo(() => {}, [checkList, btns, total, remaining]);
  ...
  const pageHandler = (page) => {
    ...
    axios
      .get(
        `https://exam-portal-by-hritik-sanam.herokuapp.com/student/questions`,
        {
          headers: { Authorization: `Bearer ${token}` },
          params: {
  ...
  const checkHandler = (quesNo, optionIndex) => {
    ...
    setCheckList(
      checkList.map((currentValue, index) => {
        if (quesNo === index) {
  ...
  const submitHandle = () => {
    ...
    axios
      .post(
        `https://exam-portal-by-hritik-sanam.herokuapp.com/student/exam`,
        submitData,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const saveAnswer = () => {
    ...
    axios
      .post(
        "https://exam-portal-by-hritik-sanam.herokuapp.com/student/answer",
        postData,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  return (
    <>
      <FullScreen handle={screen1} className="bg-white" onChange={reportChange}>
        <div>
              ...
              <div className="row py-2">
                ...
                <div className="col timer my-4 mr-4">
                  <p>
                    Remaining time:{" "}
                    <button>
                      <ReactCountdown remaining={remaining} />
                    </button>{" "}
                  </p>
                </div>
              </div>
          ...
          <div className="row mr-0">
            <div className="col-md-8 mb-2  section-questions">
                      ...
                      {ques.map((Q, i) => (
                        <div className="questions" key={i}>
                          <div className="question-section">
                            ...
                            <div className="question-text my-3 mx-5">
                              {Q.question}
                            </div>
              ...
              <div className="buttons questions-button mt-4 center-content-in-div">
                ...
                <button className="btn btn-info" onClick={saveAnswer}>
                  Save
                </button>
                ...
                {btns.map((color, i) => (
                  <div className="" key={i}>
                    <button
                      className="btn ml-2"
                      style={{
                        backgroundColor: i === pageNo ? "#20B2AA" : color,
                      }}
      ...
      <Modal show={modalShow}>
        ...
        <Modal.Body>
          <h4>You can toggle full screen 5 time only</h4>
          <p>You have toggled full screen : {toggleNo}</p>
        </Modal.Body>
        <Modal.Footer>
